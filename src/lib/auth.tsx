...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const value = useMemo<AuthContextValue>(() => ({
    ...
    async login(email, password, expectedRole) {
      try {
        const result = await loginFn({ data: { email, password, expectedRole } });
    ...
    async register(input) {
      try {
        ...
        console.error(error);
        return { ok: false, message: "We couldn't create your account right now. Please try again." };
      }
    },
    async requestPasswordReset(email) {
      try {
        ...
        console.error(error);
        return { ok: false, message: "We couldn't prepare the password reset right now." };
      }
    },
    async resetPassword(token, password) {
      try {
        ...
        console.error(error);
        return { ok: false, message: "We couldn't reset your password right now." };
      }
    },
    async logout() {
      try {
