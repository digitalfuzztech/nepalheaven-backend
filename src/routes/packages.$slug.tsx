...
import { Check, Clock, Signal, Star, X, Users } from "lucide-react";
import { getPackageBySlugFn, getPackagesFn, getTestimonialsFn } from "@/lib/content.functions";
...
export const Route = createFileRoute("/packages/$slug")({
  loader: async ({ params }) => {
    const [pkg, packages, testimonials, customerBookingResult] = await Promise.all([
  ...
  head: ({ loaderData, params }) => {
    if (!loaderData) {
      return { meta: [{ title: "Package unavailable | Nepal Heaven" }, { name: "robots", content: "noindex" }] };
    }
...
function PackageDetail() {
  const { pkg: p, packages, testimonials, customerBookingResult } = Route.useLoaderData();
  const customerBooking = customerBookingResult.ok ? customerBookingResult.booking : null;
  ...
  return (
    <>
      <PageHero
        ...
        title={p.title}
        description={p.short}
        crumbs={[{ label: "Home", to: "/" }, { label: "Packages", to: "/packages" }, { label: p.title }]}
      />
        ...
        <div className="space-y-20">
          <Reveal as="section">
            <dl className="grid grid-cols-2 gap-6 rounded-3xl border border-border bg-card p-7 sm:grid-cols-4">
              {[
                ...
                { icon: Star, k: "Rating", v: `${p.rating.toFixed(1)} (${p.reviews})` },
              ].map(({ icon: Icon, k, v }) => (
            ...
            <p className="mt-5 text-base leading-relaxed text-muted-foreground">
              {p.short} Across {p.days} days in {p.destination}, this itinerary balances the landmark moments with
              enough unscheduled time to let the place register. Group departures cap at twelve travellers; private
              versions run on any date you choose.
            </p>
          ...
          <Reveal as="section">
            ...
            <div className="mt-8 grid gap-5 sm:grid-cols-3">
              {p.tiers.map((tier, i) => (
                ...
                >
                  <p className={i === 1 ? "eyebrow" : "text-xs font-bold uppercase tracking-[0.2em] text-muted-foreground"}>
                    {tier.name}
                  </p>
                  <p className="mt-4 font-[family-name:var(--font-display)] text-3xl font-semibold">
                    ${tier.price.toLocaleString()}
                  </p>
                  <p className={i === 1 ? "mt-2 text-sm text-primary-foreground/70" : "mt-2 text-sm text-muted-foreground"}>
                    per person · {tier.note}
                  </p>
                  <Link
                    to={customerBooking ? "/account/bookings/$reference" : "/book/$slug"}
                    params={customerBooking ? { reference: customerBooking.reference } : { slug: p.slug }}
                    className={
                      i === 1
            ...
            <div className="mt-6 grid grid-cols-2 gap-4 sm:grid-cols-3">
              {packages.slice(0, 6).map((x, i) => (
                <figure key={i} className="zoom-media aspect-[4/3] overflow-hidden rounded-2xl">
                  <img src={x.image} alt={`${p.title} gallery ${i + 1}`} loading="lazy" className="h-full w-full object-cover" />
                </figure>
              ))}
            </div>
          </Reveal>
          ...
          <Reveal as="section">
            ...
            <ul className="mt-8 grid gap-5 sm:grid-cols-2">
              {testimonials.slice(0, 4).map((t) => (
                <li key={t.name} className="rounded-3xl border border-border bg-card p-7">
          ...
          <Reveal as="section">
            ...
            <div className="mt-6">
              <FaqAccordion
                items={[
                  { q: "What deposit is required?", a: "The current minimum advance and balance deadline are calculated from Nepal Heaven's booking settings and shown clearly during checkout." },
                  { q: "Is insurance mandatory?", a: "Yes. Comprehensive travel insurance including helicopter evacuation up to your maximum trip altitude is required for all travellers." },
                  { q: "Can I extend the trip?", a: "Absolutely — most travellers add Pokhara, Chitwan or a Kathmandu heritage extension. We arrange it in the same booking." },
                  { q: "What is the cancellation policy?", a: "Cancellation policy is package-specific. The applicable fee percentage is snapshotted when a qualifying payment creates the booking." },
                ]}
              />
            </div>
          </Reveal>
        ...
        <aside className="lg:sticky lg:top-28 lg:self-start">
          <div className="glass-card rounded-3xl p-7">
            <div className="flex items-end justify-between gap-3">
              <div>
                <p className="text-xs font-bold uppercase tracking-[0.18em] text-muted-foreground">From</p>
                <p className="font-[family-name:var(--font-display)] text-4xl font-semibold text-primary">
                  ${p.price.toLocaleString()}
                </p>
              </div>
              {p.oldPrice ? (
                <p className="pb-1 text-sm text-muted-foreground line-through">${p.oldPrice.toLocaleString()}</p>
              ) : null}
            </div>
            <p className="mt-1 text-xs text-muted-foreground">per person, twin share</p>

            <div className="mt-6 space-y-3">
              {customerBooking ? <div className="rounded-2xl border border-forest/20 bg-forest/5 p-5"><p className="text-sm font-bold text-forest">You have booked this trip</p><p className="mt-2 text-xs text-muted-foreground">Reference: {customerBooking.reference}<br />Departure: {customerBooking.departureDate}</p><Link to="/account/bookings/$reference" params={{ reference: customerBooking.reference }} className="bg-gold-gradient mt-4 block w-full rounded-2xl px-6 py-3 text-center text-sm font-bold text-gold-foreground">View my booking</Link></div> : <Link
                to="/book/$slug"
                params={{ slug: p.slug }}
      ...
      <section className="bg-sand py-24">
        <div className="container-lux">
          <SectionHeading eyebrow="You may also like" title="Other journeys travellers compare" />
          <div className="mt-12 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {related.map((x, i) => (
              <PackageCard key={x.slug} pkg={x} delay={i * 70} />
            ))}
          </div>
        </div>
      </section>
